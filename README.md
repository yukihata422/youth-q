...
- `/Users/yukihata/Documents/Playground/sync-notion.js`
  - Notionの `YQコンテンツ` データベースを読み込み、`faq-notion-synced.js` のNotion由来FAQを自動更新します。
- `/Users/yukihata/Documents/Playground/faq-notion-synced.js`
  - GitHub Actionsで生成されるNotion同期済みFAQです。読み込み時にNotion由来FAQを丸ごと差し替えます。
...
コンテンツの原本はNotionの `そのモヤモヤ、聞いて良い_コンテンツ` ページ内にある `YQコンテンツ` データベースです。データベース内の1ページを1つの質問コンテンツとして扱います。
...
ブラウザ表示用には、Notionから取り込んだ内容を `/Users/yukihata/Documents/Playground/faq-notion-synced.js` へ反映します。1件の質問は次の単位で管理します。
...
GitHub Actionsを使う場合、GitHubの `Actions` タブから `Sync Notion FAQ` を選び、`Run workflow` を押すだけで同期できます。同期後に自動コミットされ、NetlifyがGitHub連携で自動公開します。

初回だけ、GitHubリポジトリの `Settings` → `Secrets and variables` → `Actions` → `New repository secret` で、次を設定してください。

- `NOTION_TOKEN`: Notion integration token

Notion integrationには、`YQコンテンツ` データベースへの読み取り権限を付けてください。

ローカルで手動同期したい場合は、次のコマンドでサイト用データに反映できます。

```bash
NOTION_TOKEN=secret_xxx node sync-notion.js
```

同期スクリプトは、`faq-notion-synced.js` を生成します。ページ読み込み時に `source: "notion"` のFAQだけを丸ごと差し替えるため、手入力のFAQ、ティッカー、画像パネル、デザイン設定は残ります。

別のNotionデータベースを同期したい場合は、必要に応じて `NOTION_DATABASE_ID` も指定できます。

```bash
NOTION_TOKEN=secret_xxx NOTION_DATABASE_ID=35f4a4d5c3048093b9f4fb15ba5dd0d5 node sync-notion.js
```
